...
const MerchantsPage: React.FC = () => {
  ...
  const fetchMerchants = useCallback(async () => {
    setIsLoading(true);
    try {
      await fetchItem("merchants", 1, totalItemsFetched);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <>
      <HelmetProvider>
        <Helmet>
          <title>Merchants - Gestion des Commerçants</title>
          <meta
            name="description"
            content="Page de gestion des commerçants pour les utilisateurs."
          />
          <meta
            name="keywords"
            content="commerçants, gestion, utilisateurs, application"
          />
        </Helmet>
